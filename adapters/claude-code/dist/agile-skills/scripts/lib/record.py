...
# The journal's entry heading, per spec/journal-and-history.md §2.2. It lives here because it is
# a rule about a record's structure and it was written out twice — once in the loader every
# validator reads through, once in the tool that appends entries.
JOURNAL_HEADING_RE = re.compile(
    r"^##\s+(?P<when>\S+)\s+\u2014\s+(?P<skill>[a-z0-9-]+)\s+v(?P<version>\S+)"
    r"\s+\u2014\s+(?P<persona>.+?)\s*$"
)
...
class Entry:
    ...
    # `heading` is the raw heading line, so a finding can quote what is actually written;
    # `title` is its text with the hashes and surrounding space removed.
