...
| `accepted-gaps-are-dispatchable` | every type this skill is dispatched on | hard | run `.claude/agile-skills/scripts/lint-documents --rule accepted-gaps-are-dispatchable --item {{item.id}}`, expect exit-zero | stay |
...
- [ ] Every accepted gap was made dispatchable in THIS execution - a question filed, or an item filed - before the closing transition. A gap that names an owner and lives only in review.md or in the item's Notes is inert, because the orchestrator dispatches on open questions and on item status and on nothing else, so nothing will ever cause that owner to run (F-090).
