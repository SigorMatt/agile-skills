# Contract — answer-questions v0.6.1
...
| `tracker/items/{{item.id}}/artifacts/plan.md` | no | most answers land here, and an answer that does not reach an artifact has not been given; it also carries the invalidation set, which is where a document this answer changes or falsifies is accounted for while the item is mid-flight |
...
| `every document this answer must be propagated into` | no | an answer that does not reach the document the asker will re-read has not been given, and the sentence it lands in carries the obligation of its claim kind - a citation that resolves, or, for a claim over a family, the enumeration |
...
| `the documents this answer is propagated into - docs/product/, docs/architecture/overview.md, docs/process/ - each with a version bump and a change-log row` | file | conditional |
...
- [ ] Every claim this execution wrote into a document carries what its kind owes - a resolving citation for a cited fact, and for a quantified claim the set, the enumeration with its command's output, the members and a verdict per member (spec/doc-header.md section 4a).
- [ ] No Engagement state section was written or amended. Where an answer falsified one, the question's Consequences names the document and its section and records that the ending owns it, and the item's plan, where it has one, carries the entry disposed owned-by-ending.
- [ ] Every document this execution wrote on an item's branch is accounted for in that item's invalidation set, so the set still names every path under docs/ in the branch diff (spec/dor-dod.md D7).
