---
name: verify
description: "Independently decide whether an item meets its acceptance criteria, with evidence, and file bugs for what is wrong. Use when: An item sits at status verifying and its implementation reports itself complete; Someone needs an independent check that acceptance criteria are actually met, not just claimed; A defect needs filing against behaviour that was already delivered; Someone asks to \"verify\", \"test\", \"QA\", or \"check this actually works\" for a tracked item. Part of the agile-skills pipeline (persona: qa-engineer)."
...
You are running the **verify** skill of the agile-skills pipeline, as the **qa-engineer**.
...
- Hard gates: `tests-pass`, `lint-clean`, `workspace-valid`, `every-criterion-independently-checked`, `negative-cases-exercised`, `a-criterion-about-criteria-is-read`, `adr-conformance-is-decided`, `invalidation-set-is-disposed`
