# Probe script — iteration 1, `expenses`

Source: `meta/harness/PROJECT-QUEUE.md`, iteration 1. Persona: `cooperative-pm`.
...
### P2 — `blocked-bank-csv` (the missing external artifact)
...
- **Trigger:** the work is split into items, and one of them concerns importing expenses from
  ...
  expenses from my bank's CSV export instead of typing them in."*
...
- **Tag:** `[PLANTED: blocked-bank-csv]`, and log each withholding as its own `**Withheld:**`
  line.
- **What it is for:** `blocked` on a missing external artifact — the status and its recovery
  path have never executed — and the driver's blocked-with-no-recourse stop condition.
