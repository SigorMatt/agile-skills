# Probe script — iteration 1c, `expenses`

Source: `meta/harness/PROJECT-QUEUE.md`, iteration 1, re-run 1c. Runs 1 and 1b never reached
`blocked`: 1's sim accepted a deferral; 1b's team never asked a question the probe could ride
in on, and the driver gives the sim no turn without open questions. 1c puts the import in the
opening idea so the item exists from intake. Persona: `cooperative-pm`.
