...
| **EP-002** | observations and proposals of the wrong shape | an uncited observation, a citation that does not resolve, a proposal numbered `F-101` and marked `filed`, a classification outside the closed set, a `toolkit-defect` with neither counterfactual nor recurrence, a missing `Severity:`, and a proposal count that contradicts the frontmatter |
